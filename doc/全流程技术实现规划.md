...
- 运行与托管（当前状态 → 目标）：
  ...
  - 后端 API：Cloudflare Workers（`wrangler` 管理），与前端同域/子路径部署（如 `/api/*`）。【最小实现已落地：artworks/users/feed 路由、health/redis ping；DEV 鉴权通行】
  - 静态资产：由 Pages 托管，图片/视频原文件与缩略图存于 R2。【未启动】
  - 任务调度：Workers Cron Triggers。【wrangler 已配置 cron，占位可用；业务任务未实现】

- 基础服务（目标，当前进度）：
  - 鉴权：Clerk（前端 SDK + Workers 后端校验 JWT）。【DEV_MODE=1 自动通行，真实校验未接入；Secrets 已注入】
  - 数据库：Cloudflare D1（`users`, `artworks`, `artworks_like`, `artworks_favorite`）。【wrangler 绑定已配置；当前以内存存根代替查询】
  - 缓存：Upstash Redis（HTTP API），Key 规范见蓝图。【Secrets 已注入；最小实现用内存计数，后续替换为 Upstash】
...
- 前端 UI（当前状态）：
  ...
  - 数据请求：客户端使用 `swr`；`NEXT_PUBLIC_USE_MOCK=0` 时已默认走 `/api/*`，相对路径自动重写到 `NEXT_PUBLIC_API_BASE_URL`；`authFetch` 支持 DEV JWT 回退，Clerk 注入后续开启。
...
### Workers（`wrangler.toml`）（已存在，Secrets 已配置）
```toml
name = "ai-social-api"
main = "src/index.ts"
...
- 费用与配额：AI 生成/存储/流量需预算与告警。
 现状：Workers 最小版已连通（artworks/users/feed/detail/like/favorite/publish）；返回体已对齐前端类型；鉴权/持久化仍为 DEV/内存存根，短期可用于联调；建议下一阶段接入真实 Clerk 校验、D1/Upstash/R2 封装与上传流程。
