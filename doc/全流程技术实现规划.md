...
- 运行与托管（当前状态 → 目标）：
  ...
  - 后端 API：Cloudflare Workers（`wrangler` 管理），与前端同域/子路径部署（如 `/api/*`）。【最小实现已落地：artworks/users/feed/detail 路由、health/redis ping；DEV 鉴权通行；详情接口已修复；错误中间件与参数校验就绪】
...
- 基础服务（目标，当前进度）：
  ...
  - 数据库：Cloudflare D1（`users`, `artworks`, `artworks_like`, `artworks_favorite`）。【wrangler 绑定已配置并可查询；本地已写入测试数据】
  - 缓存：Upstash Redis（HTTP API），Key 规范见蓝图。【已接入 REST 数组命令格式；DEV 仅 `DEV_MODE==='1'` 内存回退】
  - 对象存储：Cloudflare R2（`/artworks/original/:uuid`、`/artworks/thumb/:uuid`）。【上传封装已接入，并支持 `R2_PUBLIC_*` 公网 URL；缩略图与权限后续完善】
...
- 前端 UI（当前状态）：
  ...
  - 数据请求：客户端使用 `swr`；`NEXT_PUBLIC_USE_MOCK=0` 时默认走 `/api/*`；`authFetch` 支持 DEV JWT 回退。
...
- 服务封装：
  ...
  - `r2.ts`：`putObject/getObject`，支持配置化公网 URL；签名 URL 可选。
...
- 性能：
  ...
  - `like_count` 同步写入 Redis + D1；中长期可加事件式回写与一致性校验。
...
[vars]
# 开发环境建议：开启 DEV_MODE 以免鉴权与 Redis 内存回退（仅限本地）
DEV_MODE = "1"
# 生产环境：删除/置 0 DEV_MODE，并通过 wrangler secret 配置下列敏感变量
...
- 费用与配额：AI 生成/存储/流量需预算与告警。
  现状：Workers 最小版已连通（artworks/users/feed/detail/like/favorite/publish）；详情 500 已修复；返回体基本对齐前端类型（短期混合直返/包装并存）；本地 DEV 可免鉴权与 Redis 内存回退；建议下一阶段接入真实 Clerk 校验、Upstash Redis、完善 R2 上传/缩略图与错误码统一。
...
1) M1 可用最小集（当前进度约 75%）：
   - [完成] 前端页面与 UI 组件、样式体系；Hooks（like/favorite/publish/feed/favorites）。
   - [完成] Workers 最小链路：artworks/users/feed/detail/like/favorite/publish 路由、最小服务封装、Secrets 配置、DEV 鉴权。
   - [完成] 前端 SSR/RSC 已切到 `/api/*`（保留 mocks fallback）；`authFetch` 支持相对路径与 DEV JWT。
   - [待办] Clerk 校验（前后端）、D1/Upstash 真接入与缓存一致性、R2 上传与缩略图、错误码与日志规范化。
