...
- 运行与托管（当前状态 → 目标）：
  ...
  - 后端 API：Cloudflare Workers（`wrangler` 管理），与前端同域/子路径部署（如 `/api/*`）。【未启动】
  - 静态资产：由 Pages 托管，图片/视频原文件与缩略图存于 R2。【未启动】
  - 任务调度：Workers Cron Triggers。【未启动】

- 基础服务（目标，当前均未接入）：
  - 鉴权：Clerk（前端 SDK + Workers 后端校验 JWT）。【未接入】
  - 数据库：Cloudflare D1（`users`, `artworks`, `artworks_like`, `artworks_favorite`）。【未接入】
  - 缓存：Upstash Redis（HTTP API），Key 规范见蓝图。【未接入】
  - 对象存储：Cloudflare R2（`/artworks/original/:uuid`、`/artworks/thumb/:uuid`）。【未接入】
...
│  │  └─ next.config.ts
│  │
│  └─ worker-api/                  # Cloudflare Workers (Hono)【未创建】
│     ├─ src/
│     │  ├─ index.ts               # 入口，注册路由
...
- 费用与配额：AI 生成/存储/流量需预算与告警。
 - 现状：后端与基础设施未落地，短期以 mocks 驱动；建议尽快创建 `apps/worker-api` 并打通最小链路。
...
1) M1 可用最小集（当前进度约 40%）：
   - [完成] 前端页面与 UI 组件、样式体系；SSR 基础与 mocks 数据。
   - [待办] 接入 Clerk、落地 Workers API（generate/upload/publish/like/favorite/feed/detail/users）与 D1/R2/Redis；替换 mocks。
